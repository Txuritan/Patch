...
/**
 * 
 * @author Txuritan - taken from UE
 */
public class CapeEventHandler
{
	private final String serverLocation = "https://raw.githubusercontent.com/Txuritan/Txuritan.github.io/master/img/capes/capes.txt";
		...
		try
		{
			...
			while ((str = br.readLine()) != null)
			{
				if (!str.startsWith("--") && !str.isEmpty())
				{
					...
					{
						Patch.logger.severe("[capes.txt] Syntax error on line " + linetracker + ": " + str);
					}
				}
				linetracker++;
			}
	...
	private class CloakThread implements Runnable
	{
		...
		public void run()
		{
			...
			{
				Patch.logger.fine("Failed to load capes");
				e.printStackTrace();
			}
		}
	}
